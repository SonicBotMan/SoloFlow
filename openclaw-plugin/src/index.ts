            ...
            async onTemplateFound(template) {
              ...
              if (template.type === "skill" && template.pattern) {
                try {
                  ...
                  const triggers = (template.triggers || []).join(", ");
                  const tools = [...(template.tools_required || []), ...(template.tools_optional || [])].join(", ");
                  const frontmatter = [
                    "---",
                    `name: ${template.name}`,
                    `slug: ${slug}`,
                    ...
                    `metadata: {"evolved": true, "source": "soloflow-evolution", "tools": "${tools}"}`,
                    "---",
                    "",
                  ].join("\n");
                  const body = [
                    `## ${template.name}`,
                    "",
                    template.description || "Auto-evolved skill pattern.",
                    "",
                    "### Triggers",
                    triggers ? triggers.split(", ").map(t => `- ${t.trim()}`).join("\n") : "- (auto-detected)",
                    "",
                    "### Tools Required",
                    (template.tools_required || []).length ? template.tools_required.map(t => `- \`${t}\``).join("\n") : "- None specific",
                    "",
                    "### Pattern",
                    "",
                    template.pattern,
                    "",
                    "---",
                    `*Auto-generated by SoloFlow Skill Evolution on ${new Date().toISOString().split('T')[0]}*`,
                  ].join("\n");
