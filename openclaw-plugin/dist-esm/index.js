...
function routeToDiscipline(task) {
...
__export(hooks_exports, {
  ...
  onWorkflowFail: () => onWorkflowFail,
  onWorkflowStart: () => onWorkflowStart,
  registerBuiltinHooks: () => registerBuiltinHooks,
...
function onWorkflowStart(ctx) {
  metrics.started++;
  console.log(
    `[SoloFlow] workflow started: ${ctx.workflow?.name ?? ctx.workflow?.id ?? "unknown"} (${ctx.workflow?.steps.size ?? 0} steps)`
  );
}
function onWorkflowComplete(ctx) {
  metrics.completed++;
  ...
  console.log(
    `[SoloFlow] workflow completed: ${wf?.name ?? wf?.id ?? "unknown"} duration\u2248${metrics.totalDurationMs}ms`
  );
}
function onWorkflowFail(ctx) {
  metrics.failed++;
  console.error(
...
function onStepComplete(ctx) {
  console.log(
    `[SoloFlow] step completed: ${ctx.step?.name ?? ctx.step?.id ?? "unknown"} in workflow ${ctx.workflow?.id ?? "?"}`
  );
}
function onStepError(ctx) {
  console.error(
    `[SoloFlow] step failed: ${ctx.step?.name ?? ctx.step?.id ?? "unknown"} in workflow ${ctx.workflow?.id ?? "?"}`,
    ctx.step?.error ?? ctx.metadata?.["error"] ?? ""
  );
}
function registerBuiltinHooks(system = hookSystem) {
  const unsubs = [
...
function registerHooks(_api, _system = hookSystem) {
  return () => {
  };
}
function unregisterHooks(_api) {
}
var HookSystem, hookSystem, metrics;
var init_hooks = __esm({
  "src/hooks/index.ts"() {
    ...
    metrics = { started: 0, completed: 0, failed: 0, totalDurationMs: 0 };
  }
});
...
var init_working_memory = __esm({
  "src/memory/working-memory.ts"() {
    "use strict";
    DEFAULT_WORKING_CAPACITY = 256;
    WorkingMemory = class {
      store = /* @__PURE__ */ new Map();
      capacity;
      namespace;
      constructor(namespace, capacity = DEFAULT_WORKING_CAPACITY) {
        this.namespace = namespace;
        this.capacity = capacity;
      }
      set(key, value, source = "system", extra) {
...
var DEFAULT_EPISODIC_CAPACITY, DEFAULT_COMPRESSION_THRESHOLD_MS, EpisodicMemory;
var init_episodic_memory = __esm({
  "src/memory/episodic-memory.ts"() {
    "use strict";
    DEFAULT_EPISODIC_CAPACITY = 500;
    ...
    EpisodicMemory = class {
      ...
      namespace;
      constructor(namespace, capacity = DEFAULT_EPISODIC_CAPACITY, compressionThresholdMs = DEFAULT_COMPRESSION_THRESHOLD_MS) {
        this.namespace = namespace;
        this.capacity = capacity;
      ...
      storeExecution(workflow) {
        ...
        const stepSummary = Array.from(workflow.steps.values()).map((step2) => ({
          ...
          durationMs: step2.startedAt && step2.completedAt ? step2.completedAt - step2.startedAt : 0,
      ...
      computeDuration(workflow) {
        let earliest = Infinity;
        let latest = 0;
        for (const step2 of workflow.steps.values()) {
          if (step2.startedAt != null && step2.startedAt < earliest) earliest = step2.startedAt;
          if (step2.completedAt != null && step2.completedAt > latest) latest = step2.completedAt;
        }
      ...
      serializeWorkflow(workflow) {
        return {
          ...
          steps: Array.from(workflow.steps.entries()).map(([id, step2]) => ({
            id,
            name: step2.name,
            state: step2.state,
            result: step2.result,
...
var init_semantic_memory = __esm({
  "src/memory/semantic-memory.ts"() {
    ...
    SemanticMemory = class {
      ...
      forgettingConfig;
      adapter = null;
      constructor(namespace, forgettingConfig) {
        this.namespace = namespace;
      ...
      async getFacts(query) {
        const entries = this.adapter?.connected ? await this.queryFromAdapter(query) : this.queryLocal(query);
      ...
      pruneForgotten() {
        ...
        let pruned = 0;
        for (const [id, entry] of this.store) {
          const r = this.computeRetrievability(entry.createdAt, now2, entry.stability, entry.importance);
      ...
      computeRetrievability(createdAt, now2, stability, importance) {
        const elapsed = now2 - createdAt;
...
var LobsterPressBridge, InMemoryFallbackAdapter;
var init_bridge = __esm({
  "src/memory/bridge.ts"() {
    "use strict";
    LobsterPressBridge = class {
      _connected = false;
      semanticBackend = null;
      compressorBackend = null;
      dbPath;
      constructor(config) {
      ...
      async init() {
        try {
          ...
          this.semanticBackend = new mod.SemanticMemory(
            this.dbPath ? { dbPath: this.dbPath } : void 0
          );
      ...
      async querySemantic(query) {
        ...
        return results.map((r) => {
          ...
          return {
            ...
            retrievability: meta?.["retrievability"] ?? 1,
      ...
      async compressEpisodic(entries) {
        ...
        const compressed = await this.compressorBackend.compress(rawArray);
...
var init_memory = __esm({
  "src/memory/index.ts"() {
    ...
    MemorySystem = class {
      ...
      async storeWorkflowExecution(workflow) {
...
var init_types = __esm({
  "src/vector/types.ts"() {
    ...
    DEFAULT_SEARCH_OPTIONS = {
      ...
      timeDecay: { halfLifeMs: 14 * 24 * 60 * 60 * 1e3 },
...
function deserializeEmbedding(data, dimensions) {
...
var init_embedder = __esm({
  "src/vector/embedder.ts"() {
    ...
    OpenAIEmbedder = class {
      ...
      model;
      batchSize;
      constructor(apiBase, apiKey, model, dimensions, batchSize) {
  ...
  "src/vector/retriever.ts"() {
    ...
    HybridRetriever = class {
      constructor(embedder, store, options) {
        ...
        this.store = store;
        this.options = {
          rrf: { ...DEFAULT_SEARCH_OPTIONS.rrf, ...options?.rrf },
          mmr: { ...DEFAULT_SEARCH_OPTIONS.mmr, ...options?.mmr },
          timeDecay: { ...DEFAULT_SEARCH_OPTIONS.timeDecay, ...options?.timeDecay },
          normalization: { ...DEFAULT_SEARCH_OPTIONS.normalization, ...options?.normalization },
          defaultTopK: options?.defaultTopK ?? DEFAULT_SEARCH_OPTIONS.defaultTopK
        };
      }
      embedder;
      store;
      ...
      async search(query, topK) {
        ...
        const scored = mmrCandidates.map((cand) => {
          ...
          const timeDecay = this.computeTimeDecay(doc?.createdAt ?? now2, now2);
      ...
      reciprocalRankFusion(ftsResults, vectorResults) {
      ...
      mmrDeduplicate(candidates, embeddingMap, topK) {
        ...
        while (selected.length < topK && remaining.length > 0) {
          ...
          for (let i = 0; i < remaining.length; i++) {
            ...
            for (const sel of selected) {
              const sim = cosineSimilarity2(cand.embedding, sel.embedding);
      ...
      computeTimeDecay(createdAt, now2) {
...
var init_indexer = __esm({
  "src/vector/indexer.ts"() {
    ...
    init_embedder();
    VectorIndexer = class {
      constructor(embedder, dbPath = ":memory:") {
        this.embedder = embedder;
        this.dbPath = dbPath;
      }
      embedder;
    ...
    `);
      ...
      async indexStep(step2, workflowId) {
        this.ensureReady();
        const content = this.stepToContent(step2);
        const embedding = await this.embedder.embed(content);
        const id = `step_${workflowId}_${step2.id}`;
        const now2 = Date.now();
        const doc = {
          id,
          workflowId,
          ...
          title: step2.name,
          embedding: { data: serializeEmbedding(embedding), dimensions: embedding.length },
          createdAt: step2.startedAt ?? now2,
          updatedAt: step2.completedAt ?? now2,
          metadata: { discipline: step2.discipline, state: step2.state, config: step2.config }
        };
        this.upsertDocument(doc);
        return id;
      }
      async indexWorkflowWithSteps(workflow) {
        const ids = [];
        const wfId = await this.indexWorkflow(workflow);
        ids.push(wfId);
        for (const step2 of workflow.steps.values()) {
          const stepId = await this.indexStep(step2, workflow.id);
          ids.push(stepId);
        }
        return ids;
      }
      ...
      async deleteByStep(workflowId, stepId) {
        this.ensureReady();
      ...
      getStats() {
        this.ensureReady();
        const total = this.db.prepare("SELECT COUNT(*) as count FROM vector_documents").get().count;
        const workflows = this.db.prepare("SELECT COUNT(*) as count FROM vector_documents WHERE doc_type = 'workflow'").get().count;
    ...
    `).all(ftsQuery, limit);
      ...
      async vectorSearch(embedding, limit) {
        ...
        for (const row of rows) {
          ...
          const sim = cosineSimilarity2(embedding, stored);
    ...
    `).run(
      ...
      workflowToContent(workflow) {
        const stepDescriptions = Array.from(workflow.steps.values()).map((s) => `${s.name} (${s.discipline}): ${this.configToText(s.config)}`).join("; ");
        return [
      ...
      stepToContent(step2) {
        return [
          ...
          `Discipline: ${step2.discipline}`,
          `State: ${step2.state}`,
          this.configToText(step2.config),
          step2.error ? `Error: ${step2.error}` : "",
          step2.result ? `Result: ${JSON.stringify(step2.result)}` : ""
        ].filter(Boolean).join("\n");
      }
      configToText(config) {
        try {
...
var init_vector = __esm({
  "src/vector/index.ts"() {
    ...
    VectorSearchSystem = class {
      ...
      async searchWorkflows(query, topK) {
        this.ensureReady();
        const results = await this.retriever.search(query, topK);
        const workflowIds = new Set(
          results.filter((r) => !r.stepId).map((r) => r.workflowId)
        );
        ...
        return Array.from(workflowIds).map((id) => ({
          ...
          dag: { nodes: /* @__PURE__ */ new Map(), edges: [], layers: [] },
          state: "completed",
          currentSteps: [],
          createdAt: Date.now(),
          updatedAt: Date.now(),
      ...
      async searchSteps(query, topK) {
        ...
        return stepResults.map((r) => ({
          ...
          step: {
            id: r.stepId,
            name: r.title,
            discipline: r.metadata["discipline"] ?? "quick",
            dependencies: [],
            config: r.metadata["config"] ?? {},
            state: r.metadata["state"] ?? "completed",
      ...
      async search(query) {
        this.ensureReady();
        const resolvedQuery = typeof query === "string" ? { text: query, mode: "auto" } : query;
        ...
        const workflows = Array.from(workflowIds).map((id) => {
        ...
        const steps = results.filter((r) => r.stepId).map((r) => ({
          step: {
            id: r.stepId,
            name: r.title,
            discipline: r.metadata["discipline"] ?? "quick",
      ...
      async removeFromIndex(workflowId) {
        this.ensureReady();
      ...
      async searchWithMemory(query, topK) {
        this.ensureReady();
...
var require_schema2 = __commonJS({
  "node_modules/yaml/dist/schema/json/schema.js"(exports) {
    ...
    var schema = [map.map, seq.seq].concat(jsonScalars, jsonError2);
    exports.schema = schema;
  }
});
  ...
  const migrations = [
    ...
    },
    {
      ...
      up: (db2) => {
        ...
        `);
        db2.prepare("INSERT OR IGNORE INTO _schema_migrations (version, applied_at) VALUES (?, ?)").run(3, Date.now());
      }
    },
    {
      version: 4,
...
// src/evolution/skill-inventory.ts
var skill_inventory_exports = {};
__export(skill_inventory_exports, {
...
var init_skill_inventory = __esm({
  "src/evolution/skill-inventory.ts"() {
    "use strict";
    SkillInventory = class {
      db;
      skillDirs;
      constructor(db) {
        this.db = db;
        this.skillDirs = [
          path2.join(os.homedir(), ".openclaw", "workspace", "skills"),
          path2.join(os.homedir(), ".npm-global", "lib", "node_modules", "openclaw", "skills")
        ];
      }
      ...
      scan() {
        ...
        const existing = new Map(
          this.db.prepare("SELECT id, name, location FROM skills_inventory").all().map((r) => [r.id, r])
        );
        ...
        `).run(
            ...
            this.db.prepare("UPDATE skills_inventory SET last_scanned_at=? WHERE id=?").run(now2, skill.id);
          }
        }
        const scannedIds = new Set(scanned.map((s) => s.id));
        const removed = this.db.prepare("SELECT id FROM skills_inventory").all().filter((r) => !scannedIds.has(r.id)).length;
        if (scanned.length > 0) {
          this.db.prepare(`DELETE FROM skills_inventory WHERE id NOT IN (${scanned.map(() => "?").join(",")})`).run(...scanned.map((s) => s.id));
        }
        ...
        for (const dir of this.skillDirs) {
          ...
          try {
            ...
            for (const entry of entries) {
              ...
              const skillPath = path2.join(dir, entry.name);
      ...
      parseSkillDir(dir, fallbackName) {
        const skillFile = path2.join(dir, "SKILL.md");
      ...
      extractFrontmatter(content, key) {
      ...
      recordUsage(skillId, toolName, success, durationMs) {
        try {
          this.db.prepare(`
    ...
    `).all(limit);
      ...
      getCombinationPatterns(windowMs = 3e5) {
        const rows = this.db.prepare(`
...
// src/evolution/skill-analyzer.ts
var skill_analyzer_exports = {};
__export(skill_analyzer_exports, {
  SkillAnalyzer: () => SkillAnalyzer
});
var SkillAnalyzer;
var init_skill_analyzer = __esm({
  "src/evolution/skill-analyzer.ts"() {
    "use strict";
    SkillAnalyzer = class {
      skillInventory;
      db;
      api;
      constructor(skillInventory, db, api) {
      ...
      analyzeUsagePatterns() {
        ...
        const usedIds = new Set(recentlyUsed.map((r) => r.skill_id));
      ...
      analyzeOpportunities() {
        ...
        for (let i = 0; i < skills.length; i++) {
          for (let j = i + 1; j < skills.length; j++) {
            ...
            const wordsA = new Set(a.description.toLowerCase().split(/\s+/).filter((w) => w.length > 4));
...
// src/api/router.ts
function matchPattern(pattern, path4) {
    ...
    Router = class {
      ...
      matchRoute(method, path4) {
        for (const route of this.routes) {
          ...
          const params = matchPattern(route.pattern, path4);
          if (params !== null) {
            return { ...route, params };
          }
        }
      ...
      buildMiddlewareChain(finalHandler) {
        let chain = (req) => Promise.resolve(finalHandler(req));
        for (let i = this.middlewares.length - 1; i >= 0; i--) {
          const mw = this.middlewares[i];
          const next = chain;
          chain = (req) => mw(req, () => next(req));
        }
        return chain;
      }
    };
...
var init_websocket = __esm({
  "src/api/websocket.ts"() {
    ...
    WebSocketServer = class {
      ...
      nextId = 0;
      init() {
        this.eventListener = this.workflowService.subscribe((event) => {
      ...
      handleMessage(wsId, raw) {
        ...
        switch (msg.type) {
          ...
          case "ping":
            ...
            break;
          default:
            this.sendTo(wsId, { type: "error", message: `Unknown message type: ${msg.type}` });
        }
      }
      get connectionCount() {
...
function createWorkflowRoutes(services, _templates) {
  return {
    async list(req) {
    ...
    async update(req) {
      ...
      if (body.metadata !== void 0) workflow.metadata = body.metadata;
...
function asWorkflowId2(s) {
  return s;
}
function buildLogsFromStatus(status) {
  const logs = [];
  logs.push({
...
function createExecutionRoutes(services) {
  return {
    async getStatus(req) {
      const id = asWorkflowId2(req.params["id"] ?? "");
...
function asStepId2(s) {
  return s;
}
function createTemplateRoutes(templates) {
  return {
    async list(_req) {
    ...
    async create(req) {
      ...
      const template = {
        ...
        steps: body.steps.map((s, i) => ({
          id: asStepId2(s.id ?? `step-${i}`),
          name: s.name,
          discipline: s.discipline,
          dependencies: s.dependencies.map(asStepId2),
          config: s.config
        }))
      };
...
async function hmacSign(payload, secret, algo) {
...
function createCompositeAuthMiddleware(jwtOptions, apiKeyOptions) {
...
__export(api_exports, {
  ...
  registerApiWithPlugin: () => registerApiWithPlugin
});
function createApiServer(services, config) {
...
function errorHandlingMiddleware() {
  return async (_req, next) => {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      const status = isHttpError2(err) ? err.status : 500;
...
function isHttpError2(err) {
  return err instanceof Error && "status" in err && typeof err.status === "number";
}
function registerApiWithPlugin(api, services, config) {
...
function getReadySteps(dag, completed, running) {
  const ready = [];
...
var InvalidTransitionError = class extends Error {
  constructor(workflowId, from, to) {
...
var DEFAULT_WORKFLOW_TIMEOUT_FACTOR = 5;
var Semaphore = class {
  constructor(max) {
    this.max = max;
  }
...
var Scheduler = class {
  constructor(workflowService, options) {
    this.workflowService = workflowService;
  ...
  async execute(workflowId, api) {
    ...
    status.progress = totalSteps > 0 ? (completed.size + failed.length) / totalSteps : 1;
    return {
      workflowId,
  ...
  async runLoop(workflow, api, completed, failed, running, status, stepResultsMap) {
    ...
    while (true) {
      ...
      const promises = executable.map(
        (stepId) => this.runManagedStep(
          workflow,
          stepId,
          api,
          completed,
    ...
    status.state = failed.length > 0 && completed.size === 0 ? "failed" : "completed";
    status.updatedAt = Date.now();
  }
  /**
  ...
  async runManagedStep(workflow, stepId, api, completed, failed, failedStepIds, running, semaphore, status, stepResultsMap) {
    ...
    status.updatedAt = Date.now();
    const step2 = workflow.steps.get(stepId);
    if (!step2) {
  ...
  findStep(stepId) {
    for (const workflow of this.workflowService.list()) {
      const step2 = workflow.steps.get(stepId);
      if (step2) return { workflow, step: step2 };
    }
    return void 0;
  }
};
...
var researchTemplate = {
  name: "Research Workflow",
  description: "Search the web for a topic, analyze the findings with a deep-reasoning agent, and produce a written summary.",
  steps: [
    {
      ...
      config: {
        tool: "web-search",
        prompt: "Search the web for the latest information on the given topic. Return the top 5 most relevant results with titles, URLs, and brief summaries.",
        topicVar: "topic",
        maxResults: 5
      }
    },
    {
      ...
      config: {
        tool: "data-analysis",
        prompt: "Analyze the search results from the previous step. Identify key themes, compare different viewpoints, highlight contradictions, and extract actionable insights.",
    ...
    },
    {
      ...
      config: {
        tool: "writer",
        prompt: "Based on the analysis, write a comprehensive research summary in markdown with an executive summary, key findings, and conclusions.",
...
var contentTemplate = {
  ...
  description: "Generate ideas, write content, create visuals, and prepare for publishing \u2014 end-to-end content production.",
  steps: [
    {
      ...
      config: {
        tool: "idea",
        prompt: "Generate 5 creative content ideas for the given topic. For each idea provide a title, 2-sentence description, target audience, and suggested format.",
        topicVar: "topic",
        count: 5
      }
    },
    {
      ...
      config: {
        tool: "writer",
        prompt: "Write full content based on the selected idea with a compelling introduction, well-structured body, and strong conclusion.",
    ...
    },
    {
      ...
      config: {
        tool: "visual",
        prompt: "Design a cover image or thumbnail for the content. The visual should capture the core theme and be eye-catching.",
    ...
    },
    {
      ...
      config: {
        tool: "publisher",
        prompt: "Format the content and visual into a publishable package. Generate metadata, apply final formatting, and produce a ready-to-publish output.",
...
var codeReviewTemplate = {
  name: "Code Review Workflow",
  description: "Automated code review: reads changed files, analyzes for issues using deep reasoning, and generates a structured review report.",
  steps: [
    {
      id: step("review-read"),
      name: "Read Code",
      ...
      config: {
        tool: "code-reader",
        prompt: "Read and extract the code changes. Fetch the diff for the specified pull request or file paths.",
        repoVar: "repo",
        prVar: "pr",
        includeContext: true,
        contextLines: 5
    ...
    },
    {
      ...
      config: {
        tool: "analyzer",
        prompt: "Perform a thorough code review analysis checking for bugs, security vulnerabilities, performance issues, and anti-patterns.",
    ...
    },
    {
      ...
      config: {
        tool: "reporter",
        prompt: "Generate a comprehensive code review report organized by severity with file locations, suggested fixes, and an overall assessment.",
...
function dagToYaml(dag, metadata) {
  const steps = [];
  for (const [id, node] of dag.nodes) {
    steps.push({
      id,
      ...
      depends_on: node.dependencies.map((d) => d)
    });
  }
  const doc = {
  ...
  return (0, import_yaml.stringify)(doc, { indentSeq: true, lineWidth: 0 });
}
function yamlToDag(yaml) {
  let parsed;
  try {
  ...
  const root = parsed;
  const validDisciplines = /* @__PURE__ */ new Set([
    "deep",
    "quick",
    "visual",
    "ultrabrain"
  ...
  for (const raw of rawSteps) {
    ...
    const deps = Array.isArray(raw["depends_on"]) ? raw["depends_on"].map((d) => String(d)) : [];
    nodes.set(id, {
      id,
      dependencies: deps,
      discipline,
  ...
  const metadata = {
    ...
    description: typeof root["description"] === "string" ? root["description"] : void 0,
    version: typeof root["version"] === "string" ? root["version"] : void 0,
    author: typeof root["author"] === "string" ? root["author"] : void 0
  };
  return { dag, metadata };
}
function validateWorkflow(dag) {
  const errors = [];
  const warnings = [];
  ...
  for (const id of nodeIds) {
    ...
    if (color.get(id) === WHITE && dfs(id)) {
      errors.push(
        `Cycle detected: ${path4.map((s) => s).join(" \u2192 ")} \u2192 ${path4[0] ?? id}`
      );
    }
  }
  for (const [id, node] of dag.nodes) {
  ...
  const referencedAsTarget = /* @__PURE__ */ new Set();
  ...
  if (dag.nodes.size > 1) {
    for (const [id, node] of dag.nodes) {
      if (node.dependencies.length === 0 && !dependedUpon.has(id)) {
        warnings.push(
          `Step '${id}' is disconnected (no dependencies and nothing depends on it)`
        );
      }
    }
  }
  for (const edge of dag.edges) {
    if (!nodeIds.has(edge.from)) {
      errors.push(
        `Edge references non-existent source '${edge.from}'`
      );
    }
    if (!nodeIds.has(edge.to)) {
      errors.push(
        `Edge references non-existent target '${edge.to}'`
      );
    }
  }
  return {
  ...
  if (dag.layers.length > 0) {
    ...
    for (let i = 0; i < dag.layers.length; i++) {
      ...
      const stepLabels = layer.map((id) => {
        const node = dag.nodes.get(id);
        const name = node?.action ?? id;
        const deps = node && node.dependencies.length > 0 ? ` [needs: ${node.dependencies.map((d) => d).join(", ")}]` : "";
        return `${name}${deps}`;
      });
      const prefix = layer.length > 1 ? `  Layer ${i} (parallel):` : `  Layer ${i}:`;
      lines.push(prefix);
...
var index_default = definePluginEntry({
  ...
  name: "SoloFlow",
  description: "DAG-based workflow orchestration with discipline agents, memory, and skill evolution",
  register(api) {
    const log = api.logger ?? {
      info: (...a) => console.log(`[${PLUGIN_NAME}]`, ...a),
      warn: (...a) => console.warn(`[${PLUGIN_NAME}]`, ...a),
      error: (...a) => console.error(`[${PLUGIN_NAME}]`, ...a),
      debug: (...a) => console.debug(`[${PLUGIN_NAME}]`, ...a)
    };
    log.info(`v${PLUGIN_VERSION} activating (safe mode)\u2026`);
    const workflowService = new WorkflowService();
    const scheduler = new Scheduler(workflowService);
    const templateRegistry = new TemplateRegistry();
    const dataDir = path3.join(os2.homedir(), ".openclaw", "data", "soloflow");
    ...
    void (async () => {
      try {
        ...
        try {
            ...
            async onTemplateFound(template) {
              ...
              if (template.type === "skill" && template.pattern) {
                try {
                  ...
                  const skillsDir = path3.join(os3.homedir(), ".openclaw", "workspace", "skills");
                  ...
                  const pushyDesc = (template.description || template.name) + (triggerKeywords ? `. Use when: ${triggerKeywords}.` : "") + (template.scope && template.scope !== "general" ? ` Scope: ${template.scope}.` : "");
                  const frontmatter = [
                    "---",
                    `name: ${template.name}`,
                  ...
                  const allTools = [.../* @__PURE__ */ new Set([...template.tools_required || [], ...template.tools_optional || []])];
                  ...
                  fs3.writeFileSync(skillFile, frontmatter + body, "utf8");
          ...
          try {
            const { SkillInventory: SkillInventory2 } = await Promise.resolve().then(() => (init_skill_inventory(), skill_inventory_exports));
            const { SkillAnalyzer: SkillAnalyzer2 } = await Promise.resolve().then(() => (init_skill_analyzer(), skill_analyzer_exports));
            skillInventory = new SkillInventory2(store.database);
            skillAnalyzer = new SkillAnalyzer2(skillInventory, store.database, { logger: log });
    ...
    void (async () => {
      try {
        const { HookSystem: HookSystem2, registerBuiltinHooks: registerBuiltinHooks2 } = await Promise.resolve().then(() => (init_hooks(), hooks_exports));
        hookSystem2 = new HookSystem2();
        unregisterBuiltinHooks = registerBuiltinHooks2(hookSystem2);
    ...
    api.registerTool(
      {
        ...
        description: "Create a SoloFlow workflow. Provide a name and an array of steps with id, name, discipline (deep|quick|visual|ultrabrain), and action.",
        ...
        async execute(_toolCallId, params) {
          try {
            ...
            const now2 = Date.now();
            const wfId = `wf_${now2.toString(36)}`;
            ...
            for (const s of steps) {
              stepMap.set(s.id, {
                ...
                discipline: s.discipline ?? "quick",
                dependencies: s.dependencies ?? [],
                config: { prompt: s.action, ...s.config ?? {} },
                state: "pending"
              });
            }
            const wf = {
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          try {
            ...
            const readyIds = workflowService.getReadySteps(wfId, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set());
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          const wf = workflowService.get(params.workflowId);
          if (!wf) {
            return {
          ...
          const execStatus = scheduler.getStatus(params.workflowId);
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          try {
            ...
            return {
              content: [{ type: "text", text: `Workflow ${wfId} cancelled` }],
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          const wfId = params.workflowId;
          const wf = workflowService.get(wfId);
          if (!wf) return { content: [{ type: "text", text: "Workflow not found" }], details: { error: true } };
          const completed = /* @__PURE__ */ new Set();
          const running = /* @__PURE__ */ new Set();
          for (const [id, step2] of wf.steps) {
            if (step2.state === "completed") completed.add(id);
            else if (step2.state === "running") running.add(id);
          }
          ...
          return {
            content: [{ type: "text", text: JSON.stringify({ readySteps, completed: completed.size, running: running.size, total: wf.steps.size }, null, 2) }],
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          ...
          const wf = workflowService.get(wfId);
          if (!wf) return { content: [{ type: "text", text: "Workflow not found" }], details: { error: true } };
          const step2 = wf.steps.get(stepId);
          if (!step2) return { content: [{ type: "text", text: `Step not found: ${stepId}` }], details: { error: true } };
          if (step2.state === "completed") return { content: [{ type: "text", text: `Step ${stepId} already completed` }], details: { error: true } };
          step2.state = params.error ? "failed" : "completed";
          step2.result = params.result ?? null;
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, _params) {
          if (!skillInventory) {
            return { content: [{ type: "text", text: "Skill inventory not available" }], details: { error: true } };
          }
          try {
            const skills = skillInventory.getAll();
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, _params) {
          if (!skillAnalyzer || !skillInventory) {
            return { content: [{ type: "text", text: "Skill system not available" }], details: { error: true } };
          }
          try {
            ...
            return { content: [{ type: "text", text: JSON.stringify({ insights, recent, combinations }, null, 2) }] };
          } catch (e) {
            return { content: [{ type: "text", text: `Error: ${e}` }], details: { error: true } };
          }
        }
    ...
    if (skillInventory) {
      ...
      api.executeTool = async (toolName, params) => {
