...
class TestPreferenceMemory:
    ...
    def test_add_preference(self, temp_db):
        ...
        assert prefs[0]["confidence"] == pytest.approx(
            0.6,
        )
    ...
    def test_min_confidence_filter(self, temp_db):
        ...
        # 添加低置信度偏好（delta=-0.25 → initial_conf=0.25 < min_confidence=0.3）
        memory.update("agent1", "low_cat", "low_val", "evidence", delta=-0.25)
